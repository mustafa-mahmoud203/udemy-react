...
function Instructor(data) {
  const getInstructors = () => {
    const listItems = [];
    for (let i = 0; i < data.allData.instructor.length; i++) {
      let val = (
        <div className={styles.instructorInfo}>
          <a className={styles.instructorName}>
            {data.allData.instructor[i].name}
          </a>
          <p className={styles.instructorIntro}>
            {data.allData.instructor[i].Intro}
          </p>

          <div className={styles.instructorCourseData}>
            <img
              className={styles.instructorImg}
              src={data.allData.instructor[i].Image}
            ></img>
            <div className={styles.CourseData}>
              <span className={styles.CourseDataVal}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="16"
                  height="16"
                  fill="currentColor"
                  className={`${styles.IconText} bi bi-star-fill`}
                  viewBox="0 0 16 16"
                >
                  <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z" />
                </svg>
                {data.allData.instructor[i].Rating} Instructor Rating
              </span>
              <span className={styles.CourseDataVal}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  ...
                  height="16"
                  fill="currentColor"
                  className={`${styles.IconText} bi bi-yelp`}
                  viewBox="0 0 16 16"
                >
                ...
                {data.allData.instructor[i].reviewsNumber} Reviews
              </span>
              <span className={styles.CourseDataVal}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  ...
                  height="16"
                  fill="currentColor"
                  className={`${styles.IconText} bi bi-person-fill`}
                  viewBox="0 0 16 16"
                >
                  <path d="M3 14s-1 0-1-1 1-4 6-4 6 3 6 4-1 1-1 1H3zm5-6a3 3 0 1 0 0-6 3 3 0 0 0 0 6z" />
                </svg>
                {data.allData.instructor[i].studentsNumber} Students
              </span>
              <span className={styles.CourseDataVal}>
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  ...
                  height="16"
                  fill="currentColor"
                  className={`${styles.IconText} bi bi-play-circle-fill`}
                  viewBox="0 0 16 16"
                >
                  <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM6.79 5.093A.5.5 0 0 0 6 5.5v5a.5.5 0 0 0 .79.407l3.5-2.5a.5.5 0 0 0 0-.814l-3.5-2.5z" />
                </svg>
                {data.allData.instructor[i].coursesNumber} Courses
              </span>
            </div>
          </div>
          <Description allData={data.allData.instructor[i]} val={false}/>
        </div>
      );
      listItems.push(val);
    }
